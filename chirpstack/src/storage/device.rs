...
pub async fn get_active_inactive(tenant_id: &Option<Uuid>) -> Result<DevicesActiveInactive, Error> {
    diesel::sql_query(r#"
        ...
        select
            ...
            coalesce(sum(case when (now() - uplink_interval) > last_seen_at then 1 else 0 end), 0) as inactive_count,
            coalesce(sum(case when (now() - uplink_interval) <= last_seen_at then 1 else 0 end), 0) as active_count
        from
...
pub async fn get_active_inactive(tenant_id: &Option<Uuid>) -> Result<DevicesActiveInactive, Error> {
    diesel::sql_query(
        ...
        select
            coalesce(sum(case when last_seen_at is null then 1 else 0 end), 0) as never_seen_count,
            coalesce(sum(case when not_seen_duration > uplink_interval then 1 else 0 end), 0) as inactive_count,
            coalesce(sum(case when not_seen_duration <= uplink_interval then 1 else 0 end), 0) as active_count
        from
            device_active_inactive
    "#,
    )
